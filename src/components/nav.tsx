    ...
    >
      <div className="container-page flex items-center justify-between h-[68px]">
        <a href="#top" className="group flex items-baseline gap-2">
          <span className="font-display text-[26px] leading-none font-medium tracking-[-0.04em] text-ink">
            Goody
          </span>
        ...
        <nav className="hidden md:flex items-center gap-9">
          {LINKS.map((l) => (
            <a
              key={l.href}
              href={l.href}
              className="group relative label-en text-ink/70 hover:text-ink transition-colors"
            >
              <span>{l.label}</span>
          ...
          <a
            href="#cta"
            className="group inline-flex items-center gap-2 bg-ink text-paper rounded-full pl-5 pr-3 py-2 text-[13px] font-medium hover:bg-coral transition-colors"
          >
            <span>登録する</span>
            <span className="grid place-items-center size-6 rounded-full bg-paper text-ink text-[10px] transition-transform group-hover:translate-x-0.5">
              →
            </span>
          </a>
        </nav>
        ...
        >
          ...
          />
          <span
            className={`absolute h-px w-6 bg-ink transition-transform duration-500 ${
              open ? "translate-y-[3px] -rotate-45" : "translate-y-[3px]"
            }`}
          />
        </button>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="md:hidden fixed inset-0 top-[68px] bg-paper border-t border-line"
          >
            <div className="container-page py-12 flex flex-col gap-7">
              {LINKS.map((l, i) => (
                ...
                >
                  <span className="font-jp-display text-[28px] font-medium text-ink">
                    {l.jp}
                  </span>
                  <span className="label-en text-ink-mute">{l.label}</span>
                </motion.a>
              ))}
              <a
                href="#cta"
                onClick={() => setOpen(false)}
                className="mt-4 inline-flex items-center justify-between bg-ink text-paper rounded-full px-6 py-4 text-base"
              >
                <span>登録する</span>
                <span>→</span>
              </a>
